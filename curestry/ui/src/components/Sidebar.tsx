  ...
  return (
    <aside className={`bg-gradient-to-br from-indigo-600 via-purple-600 to-pink-400 text-white p-6 relative overflow-visible flex flex-col border-r border-white border-opacity-20 transition-all duration-300 ${isCollapsed ? 'w-sidebar-collapsed' : 'w-sidebar-expanded'} fixed h-full`}>
      ...
      <div className="relative z-10 flex-grow">
        <div className="flex items-center justify-between mb-8">
          <div className="flex items-center">
            <div className={`w-10 h-10 ${isCollapsed ? 'mx-auto' : 'mr-3'} bg-gradient-to-br from-indigo-400 to-purple-400 rounded-lg flex items-center justify-center text-white font-bold text-xl`}>C</div>
            {!isCollapsed && (
              <div>
                <h1 className="text-2xl font-bold">Curestry</h1>
                <p className="text-sm text-white text-opacity-80 -mt-1">by <a href="https://github.com/rldyourmnd" target="_blank" rel="noopener noreferrer" className="hover:underline">Danil Silantyev</a></p>
              </div>
            )}
          </div>
        </div>
      ...
      <div className="relative z-10 mt-auto pt-6 border-t border-white border-opacity-20">
        <ul className="space-y-4">
          <SidebarExternalLink href="https://curestry.rldyourmnd.dev" icon={BookOpen} text="Documentation" isCollapsed={isCollapsed} />
          <SidebarExternalLink href="https://github.com/rldyourmnd/curestry" icon={Github} text="Code Repository" isCollapsed={isCollapsed} />
        </ul>
      </div>
